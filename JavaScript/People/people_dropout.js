...
function main()
{
  ...
  let seed = 0;
  let nn = new NeuralNet(5, 50, 3, seed);

  // 3. train network
  let lrnRate = 0.01;
  let maxEpochs = 50000;
  console.log("\nStarting training with learning rate = 0.01 drop = 0.10");
  nn.train(trainX, trainY, lrnRate, maxEpochs, 0.10);  // use dropout
  console.log("Training complete");

  // 4. evaluate model
  let trainAcc = nn.accuracy(trainX, trainY);  // no dropout after training
  let testAcc = nn.accuracy(testX, testY);     // no dropout after training
  ...
  let predicted = nn.eval(unknownNorm);  // no dropout after training
