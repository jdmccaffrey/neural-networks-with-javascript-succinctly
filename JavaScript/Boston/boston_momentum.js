...
class NeuralNet
{
  constructor(numInput, numHidden, numOutput, seed)
  {
    ...
    this.ni = numInput; 
    this.nh = numHidden;
    this.no = numOutput;  // could be a single variable
  ...
  eval(X)
  {
    ...
    // this.oNodes = U.softmax(oSums);  // no softmax 
      ...
      for (let ii = 0; ii < n; ++ii) {  // each item
        ...
        for (let i = 0; i < this.ni; ++i) {
          for (let j = 0; j < this.nh; ++j) {
            ...
            this.ihWeights[i][j] += momentRate * ihPrevWtsDeltas[i][j];
            ihPrevWtsDeltas[i][j] = delta;  
          }
        }
        ...
        for (let j = 0; j < this.nh; ++j) {
          ...
          this.hBiases[j] += momentRate * hPrevBiasDeltas[j];
          hPrevBiasDeltas[j] = delta;
        }  
        ...
        for (let j = 0; j < this.nh; ++j) {
          for (let k = 0; k < this.no; ++k) { 
            ...
            this.hoWeights[j][k] += momentRate * hoPrevWtsDeltas[j][k];
            hoPrevWtsDeltas[j][k] = delta;
          }
        }
        ...
        for (let k = 0; k < this.no; ++k) {
          ...
          this.oBiases[k] += momentRate * oPrevBiasDeltas[k];
          oPrevBiasDeltas[k] = delta;       
        }
      } // ii
